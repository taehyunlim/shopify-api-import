      ...
      if (!error && response.statusCode === 200 && body.orders.length > 0) {
        ...
        for (var i = 0; i < body.orders.length; i++) {
          ...
          for (var j = 0; j < ord.line_items.length; j++) {
            ...
            orderObj.cart_pricing.cart_price = orderObj.line_items.price - orderObj.line_items.discount;
          ...
          // TEST: Discount calcuation for OMDB
          var subtotalObj = {
            order_index: i+1,
        ...
        ordersList.forEach(function(order){
          ...
          var discountCodesAmount = order.discount_codes_amount;
          var cartSubtotal = order.cart_pricing.cart_subtotal;
          if (typeof discountCodesAmount !== 'undefined') {
            order.cart_pricing.cart_discount = discountCodesAmount/cartSubtotal;
            order.cart_pricing.cart_discount_amount = (discountCodesAmount/cartSubtotal)*order.cart_pricing.cart_price;
            order.cart_pricing.unit_price = order.cart_pricing.cart_price - order.cart_pricing.cart_discount_amount;
          }
        });
        // END OF TEST
